...
document.addEventListener("keydown", (e)=>{
    let moved = false;
    if (e.key === "ArrowLeft") moved = game.moveLeft();
    else if (e.key === "ArrowRight") moved = game.moveRight();
    else if (e.key === "ArrowUp") moved = game.moveUp();
    else if (e.key === "ArrowDown") moved = game.moveDown();
    if (moved) render();
});
 ...
 */ class Game {
   ...
   */ constructor(initialState){
    ...
    moveLeft() {
        ...
        let changed = false;
        for(let i = 0; i < this.board.length; i++){
            for(let j = 0; j < this.board[i].length; j++)if (this.board[i][j] !== result[i][j]) {
                changed = true;
                break;
            }
            if (changed) break;
    ...
    moveUp() {
        ...
        let changed = false;
        for(let i = 0; i < transposed.length; i++){
            for(let j = 0; j < transposed[i].length; j++)if (this.board[i][j] !== transposedResult[i][j]) {
                changed = true;
                break;
            }
            if (changed) break;
   ...
   */ restart() {
    ...
    addRandomTile() {
        ...
        if (Math.random() < 0.9) this.board[i][j] = 2;
        else this.board[i][j] = 4;
    }
    slideAndMerge(row) {
    ...
    checkWin() {
        for(let i = 0; i < this.board.length; i++){
            for(let j = 0; j < this.board[i].length; j++)if (this.board[i][j] === 2048) {
                this.status = "win";
                return;
            }
        }
    ...
    checkGameOver() {
        ...
        if (call === false) this.status = "lose";
    }
    transpose(matrix) {
        const result = [];
        for(let j = 0; j < matrix.length; j++){
            const words = [];
            for(let i = 0; i < matrix.length; i++)words.push(matrix[i][j]);
            result.push(words);
        }
        return result;
    }
